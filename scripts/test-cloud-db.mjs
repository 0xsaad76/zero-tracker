...
try {
  ...
  await user(A);
  const investment = {kind: 'investments', id: 'portfolio', data: {id: 'portfolio', userId: A,
    name: 'Fund', type: 'mutual_fund', startDate: '2026-09-01', reviewDay: 31,
    reminderEnabled: true, flows: [], valuations: []}};
  ...
  console.log(
    'PASS: owner isolation, anonymous denial, CAS conflicts, atomic rollback, account deletion scope, >1,000-row restore.',
  );
} finally {
  await db.close();
}
